...
def extract_info(section: configparser.SectionProxy) -> dict | None:
    """Extract URLDownload, SHA1, and SizeBytes from a section."""
    ...
    for error_type, message in error_messages.items():
        if len(errors[error_type]) > 0:
            ...
            for e in errors[error_type]:
                print("- {app_file}[{section}]: {file}".format_map(e))
...
def check_duplicates(all_info: list[dict]):
    ...
    for indices in positions.values():
        if len(indices) > 1:
            # false positive if the url is the same
...
def main():
    ...
    desc = "Check ReactOS apps for sha1 and size mismatches."
    parser = argparse.ArgumentParser(description=desc)
    parser.add_argument(
        "txt_file",
        nargs="*",
        help="INI files to check. Default is all .txt files in PWD.",
    )
    parser.add_argument(
        "-d",
        "--apps-dir",
        default="./apps",
        help="Directory for app binaries. Default is `./apps'.",
    )
    parser.add_argument(
        "-S",
        "--no-sha1",
